...
<html lang="en">
  ...
  $sql = mysql_query("SELECT contract.id AS contract_id, contract.created_date AS contract_date, contract. * , users.id AS users_id, users. * , author_bid.id AS author_bid_id, author_bid. * , uploaded_contract_file.id AS uploaded_contract_file_id, uploaded_contract_file . * 
FROM contract
INNER JOIN users ON contract.user_id = users.id
INNER JOIN author_bid ON contract.bid = author_bid.bid_id
INNER JOIN uploaded_contract_file ON uploaded_contract_file.contract_id = contract.bid
WHERE users.user_type =2 AND contract.active=1");
     ...
     while ($row = mysql_fetch_array($sql)) {
      ...
      echo "
        <tr>
          ...
          <td><a href='authorcontracts.php?action=terminate&id=$contract_id'>Terminate Contract</a></td>
